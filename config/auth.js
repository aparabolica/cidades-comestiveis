...
exports.login = function(req, res, next) {
	...
	passport.authenticate('local', function(err, user, info) {
		...
		// Unknown error
		if (err) {
			return res.json(400, messaging.error(req.i18n, err));

		// Error raised by passport
		} else if (info && info.message) {
			res.json(400, messaging.error(info.message));

		// User not found.
		} else if (!user) {
			return res.json(403, messaging.error("access_token.local.unauthorized"));
		}

		// User needs to finish migration.
		else if (user.status == 'to_migrate') {
			return res.json(400, messaging.error("access_token.local.needs_migration"));
		...
		// User doesn't have a password, because it logged before via Facebook or Google
			...
			mailer.passwordNeeded(user, req.app.locals.settings.general.serverUrl, user.callback_url, function(err){
				if (err)
					return res.json(400, messaging.error("access_token.local.error.send_email"));
				else
					return res.json(400, messaging.error("access_token.local.error.need_password"));
			});
		...
		// User needs to confirm his email, send another e-mail
			...
			req.app.locals.mailer.sendEmail('confirm_email', user.email, data, req.i18n, function(err) {
				if (err)
					...
					return res.json(400, messaging.error("access_token.local.error.needs_email_confirmation"));
			});
...
exports.logout = function(req, res, next) {
	...
	if (req.headers.authorization) {
		...
		AccessToken.findOne({_id: access_token}, function(err, at){
			...
			if (!at) return res.json(400, messaging.error("access_token.logout.error.inexistent_token"));
			at.expired = true;
		...
		res.json(400, messaging.error('access_token.logout.error.not_logged'));
	}
